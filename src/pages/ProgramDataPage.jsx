...
export default function ProgramDataPage() {
  ...
  const [pendingRestoreMeta, setPendingRestoreMeta] = useState(null);
  ...
  const handleExerciseFileSelect = (e) => {
  ...
  const handleProgramBackup = async () => {
    try {
      ...
      flash('danger', `Backup failed: ${err.message}`);
    }
  };
  ...
  const handleRestoreFileSelect = async (e) => {
    ...
    try {
      ...
      setPendingRestoreMeta(result.meta);
      setPendingRestore({ file, buffer });
      setShowRestoreConfirm(true);
    } catch (err) {
      flash('danger', `Validation failed: ${err.message}`);
    }
  };
  ...
  const confirmProgramRestore = async () => {
    ...
    try {
      ...
      flash('danger', `Restore failed: ${err.message}`);
    }
